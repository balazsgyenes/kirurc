...
Modified from https://github.com/Pointcept/Pointcept/blob/main/pointcept/models/stratified_transformer/stratified_transformer_v1m2_refine.py
...
from .stratified_transformer.kernels import KPConvLayer
...
def grid_sample(coords, batch, size, start, return_p2v=True):
    cluster = voxel_grid(coords, size, batch, start=start)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, feats, coords, offset):
        ...
        new_offset = torch.tensor(new_offset, dtype=torch.int32, device=coords.device)
...
class StratifiedTransformer(nn.Module):
    ...
    def forward(self, data: Data):
        feats, coords, batch = data.x, data.pos, data.batch
        offset = data.ptr[1:].int()
        assert batch.shape[0] == offset[-1]
        if feats is None:
    ...
    def freeze_mlp_head(self, freeze: bool, reset_weights: bool = False) -> None:
        self.classifier.requires_grad_(not freeze)
        if reset_weights:
    ...
    def freeze_last_n_upsample_layers(
        ...
        for up in reversed(self.up):
            up.requires_grad_(not freeze)
            if reset_weights:
                assert isinstance(up, TransitionUp)
                for layer in up.linear1:
                    assert isinstance(layer, (nn.Linear, nn.LayerNorm))
                    layer.reset_parameters()
                for layer in up.linear2:
                    assert isinstance(layer, (nn.Linear, nn.LayerNorm))
                    layer.reset_parameters()
