...
import kirurc_vision.models.fpt.sparse_ops as ops
...
def stride_centroids(points, counts, rows, cols, size):
    stride_centroids = ME.MinkowskiSPMMFunction().apply(
        rows, cols, counts, size, points
    )
...
def downsample_embeddings(embeddings, inverse_map, size, mode="avg"):
    ...
    if mode == "max":
        in_map = torch.arange(
            size[1], dtype=inverse_map.dtype, device=inverse_map.device
        )
...
class MaxPoolWithPoints(nn.Module):
    ...
    def forward(self, stensor, points, counts):
        ...
        cols, rows = cm.stride_map(
            stensor.coordinate_map_key, down_stensor.coordinate_map_key
        )
...
class LightweightSelfAttentionLayer(LocalSelfAttentionBase):
    def __init__(
        ...
        super(LightweightSelfAttentionLayer, self).__init__(
            kernel_size, stride, dilation, dimension=3
        )
        ...
        self.to_query = nn.Sequential(
            ME.MinkowskiLinear(in_channels, out_channels), ME.MinkowskiToFeature()
        )
    ...
    def forward(self, stensor, norm_points):
        ...
        q = (
            self.to_query(stensor)
            .view(-1, self.num_heads, self.attn_channels)
            .contiguous()
        )
        v = (
            self.to_value(stensor)
            .view(-1, self.num_heads, self.attn_channels)
            .contiguous()
        )
        ...
        attn = torch.zeros(
            (kq_map.shape[1], self.num_heads), dtype=dtype, device=device
        )
        ...
        out_F = torch.zeros(
            (len(q), self.num_heads, self.attn_channels), dtype=dtype, device=device
        )
...
class FastPointTransformer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.attn0p1 = self.LAYER(
            in_channels + self.ENC_DIM, self.INIT_DIM, kernel_size=5
        )
        ...
        self.block5 = nn.ModuleList(
            [self.BLOCK(self.PLANES[4]) for _ in range(self.LAYERS[4])]
        )
        ...
        self.block6 = nn.ModuleList(
            [self.BLOCK(self.PLANES[5]) for _ in range(self.LAYERS[5])]
        )
        ...
        self.block7 = nn.ModuleList(
            [self.BLOCK(self.PLANES[6]) for _ in range(self.LAYERS[6])]
        )
        ...
        self.block8 = nn.ModuleList(
            [self.BLOCK(self.PLANES[7]) for _ in range(self.LAYERS[7])]
        )
    ...
    def normalize_points(self, points, centroids, tensor_map):
        tensor_map = (
            tensor_map if tensor_map.dtype == torch.int64 else tensor_map.long()
        )
    ...
    def voxelize_with_centroids(self, x: ME.TensorField):
        ...
        tensor_map, field_map = cm.field_to_sparse_map(
            x.coordinate_key, out.coordinate_key
        )
        ...
        out = ME.SparseTensor(
            torch.cat([out.F, down_pos_embs], dim=1),
            coordinate_map_key=out.coordinate_key,
            coordinate_manager=cm,
        )

        norm_points_p1 = self.normalize_centroids(
            points_p1, out.C, out.tensor_stride[0]
        )
        return out, norm_points_p1, points_p1, count_p1, pos_embs

    def devoxelize_with_centroids(
        self, out: ME.SparseTensor, x: ME.TensorField, h_embs
    ):
    ...
    def forward(self, data: Data):
        ...
        if data.x is None:
            data.x = torch.ones(data.pos.shape[0], 1, device=data.pos.device)

        datas = data.to_data_list()
        coords = [d.pos for d in datas]
        feats = [d.x for d in datas]

        coords_batch, feats_batch = ME.utils.sparse_collate(
            coords, feats, dtype=coords[0].dtype
        )
        ...
        out, norm_points_p1, points_p1, count_p1, pos_embs = (
            self.voxelize_with_centroids(x)
        )
        ...
        norm_points_p2 = self.normalize_centroids(
            points_p2, out.C, out.tensor_stride[0]
        )
        ...
        norm_points_p4 = self.normalize_centroids(
            points_p4, out.C, out.tensor_stride[0]
        )
        ...
        norm_points_p8 = self.normalize_centroids(
            points_p8, out.C, out.tensor_stride[0]
        )
        ...
        norm_points_p16 = self.normalize_centroids(
            points_p16, out.C, out.tensor_stride[0]
        )
