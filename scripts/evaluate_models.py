...
def results_to_row_formatted(results):
    return [
        ...
        f"{results['splines_found'] * 100:.2f}",
        f"{results['spline_sim'] * 1000:.1f}",
        f"{results['success'] * 100:.2f}",
    ]


@hydra.main(version_base=None, config_path="../conf", config_name="evaluate_models")
def main(config: DictConfig) -> None:
    ...
    csv_folder = Path(save_csv.folder)
    ...
    for name, filter_cfg in config.methods.items():
        ...
        filters = {
            "tags": {
                "$all": list(filter_cfg.tags_all),
                "$nin": list(filter_cfg.tags_not_in),
            }
        }
        for i, run in enumerate(api.runs(f"{entity}/{project}", filters=filters)):
            ...
            # only build the model once, and assume all runs with the same config
            ...
            with open(csv_path, mode="a", newline="", encoding="utf-8") as file:
                writer = csv.writer(file)
                writer.writerow([run.id] + results_to_row(results))

            for key, value in results.items():
                aggregate_results[key].append(value)

        # aggregate results over all runs in the method
        aggregate_results = {
            key: torch.tensor(value).nanmean().item()
            for key, value in aggregate_results.items()
        }
        log.info(f"Mean results from method '{name}': {aggregate_results}")

        with open(csv_path, mode="a", newline="", encoding="utf-8") as file:
            writer = csv.writer(file)
            writer.writerow(["Mean"] + results_to_row(aggregate_results))
            writer.writerow(
                ["Mean (formatted)"] + results_to_row_formatted(aggregate_results)
            )
